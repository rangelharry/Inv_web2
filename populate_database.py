...
def popular_dados_teste():
    ...
    try:
        ...
        count_insumos = result['count'] if isinstance(result, dict) else result[0]
        ...
        if count_insumos == 0:
            ...
            categoria_id = categoria_result['id'] if isinstance(categoria_result, dict) else categoria_result[0] if categoria_result else 1
        ...
        result = cursor.fetchone()
        count_ee = result['count'] if isinstance(result, dict) else result[0]
        ...
        cursor.execute("SELECT COUNT(*) FROM insumos")
        print(f"📊 Total insumos: {cursor.fetchone()[0]}")
        
        cursor.execute("SELECT COUNT(*) FROM equipamentos_eletricos")
        print(f"📊 Total equipamentos elétricos: {cursor.fetchone()[0]}")
        
        cursor.execute("SELECT COUNT(*) FROM equipamentos_manuais")
        print(f"📊 Total equipamentos manuais: {cursor.fetchone()[0]}")
        
        cursor.execute("SELECT COUNT(*) FROM movimentacoes")
        print(f"📊 Total movimentações: {cursor.fetchone()[0]}")
